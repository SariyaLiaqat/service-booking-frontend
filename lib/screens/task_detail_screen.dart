...
if (widget.readOnly && widget.taskData != null) {
  ...
  if (t['attachments'] != null) {
    providerAttachments.addAll(
      List<String>.from(
        t['attachments'].map(
          (a) => Backend.baseUrl + "/" + a['file_path'].toString(),
        ),
      ),
    );
  }
  ...
  Future<void> createTaskWithAttachments({
    ...
    try {
      var request = http.MultipartRequest('POST', Uri.parse('${Backend.baseUrl}/tasks'));
...
request.fields['attachment_details'] = attachmentDetails ?? '';
  ...
  return GestureDetector(
    onTap: lower.endsWith(".jpg") || lower.endsWith(".jpeg") || lower.endsWith(".png") || lower.endsWith(".gif")
        ? () {
            Navigator.push(
              context,
